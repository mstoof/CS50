...
int main(int argc, string argv[])
{
    ...
    if (argc != 2)
    {
        ...
        printf("Ceasar demands only one argument.\n");
        return 1;
    }
    else
    {
        // get cipher k provided by user
        ...
        for (int i = 0; i < plaintextLen; i++)
        {
            ...
            ciphertext[i] = plainToCipher(plaintext[i], k);
        }
...
char plainToCipher(char toCipher, int key)
{
    // check if character + k is in range of ascii values reserved for alphabet letters
